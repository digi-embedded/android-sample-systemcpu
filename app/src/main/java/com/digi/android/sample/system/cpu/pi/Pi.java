...
public class Pi {
	// Implementation note: we use printf() for printing, because it flushes the
	...
	protected static abstract class AbstractBinarySplittingSeries implements
	...
	protected static class ChudnovskyBinarySplittingSeries extends
		...
		public Apfloat a(long n) throws ApfloatRuntimeException {
			Apfloat s = new Apfloat(n, Apfloat.INFINITE, this.radix), v = this.A
					.add(this.B.multiply(s));
		...
		public Apfloat p(long n) throws ApfloatRuntimeException {
			...
			if (n == 0) {
				...
				Apfloat f = new Apfloat(n, Apfloat.INFINITE, this.radix), sixf = this.SIX
						.multiply(f);

				v = sixf.subtract(this.ONE)
						.multiply(this.TWO.multiply(f).subtract(this.ONE))
						.multiply(sixf.subtract(this.FIVE));
			}
	...
	protected static class RamanujanBinarySplittingSeries extends
			AbstractBinarySplittingSeries {
		/**
		 * Default constructor.
		 */
		...
		public Apfloat p(long n) throws ApfloatRuntimeException {
			...
			if (n == 0) {
				...
				Apfloat f = new Apfloat(n, Apfloat.INFINITE, this.radix), fourf = this.FOUR
						.multiply(f);

				v = fourf.subtract(this.ONE)
						.multiply(this.TWO.multiply(f).subtract(this.ONE))
						.multiply(fourf.subtract(this.THREE));
			}
	...
	protected static class BinarySplittingPiCalculator implements Serializable {
		...
		public void r(long n1, long n2, ApfloatHolder T, ApfloatHolder Q,
				ApfloatHolder P,
				BinarySplittingProgressIndicator progressIndicator)
			...
			if (length == 1) {
				...
				T.setApfloat(Q.getApfloat().multiply(LT.getApfloat())
						.add(LP.getApfloat().multiply(T.getApfloat())));
	...
	public static class ChudnovskyPiCalculator implements Operation<Apfloat> {
		...
		public ChudnovskyPiCalculator(long precision, int radix)
				throws ApfloatRuntimeException {
			this(new BinarySplittingPiCalculator(
					new ChudnovskyBinarySplittingSeries(precision, radix)),
					precision, radix);
		}
		...
		protected ChudnovskyPiCalculator(
				BinarySplittingPiCalculator calculator, long precision,
				int radix) throws ApfloatRuntimeException {
...
//			Pi.err.println("Using the Chudnovsky brothers' binary splitting algorithm");
			...
			long terms = (long) ((double) this.precision
					* Math.log((double) this.radix) / 32.654450041768516);

			long time = System.currentTimeMillis();
			this.calculator.r(0, terms + 1, T, Q, null,
					new BinarySplittingProgressIndicator(terms));
			time = System.currentTimeMillis() - time;

//			Pi.err.println("100% complete, elapsed time " + time / 1000.0
//					+ " seconds");
//			Pi.err.printf("Final value ");
			...
			checkAlive();
			Apfloat factor = ApfloatMath.inverseRoot(new Apfloat(640320,
					this.precision, this.radix), 2);
			checkAlive();
			Apfloat pi = ApfloatMath.inverseRoot(factor.multiply(t), 1)
					.multiply(new Apfloat(53360, Apfloat.INFINITE, this.radix))
					.multiply(q);
			time = System.currentTimeMillis() - time;
...
//			Pi.err.println("took " + time / 1000.0 + " seconds");
	...
	public static class RamanujanPiCalculator implements Operation<Apfloat> {
		...
		public RamanujanPiCalculator(long precision, int radix)
				throws ApfloatRuntimeException {
			this(new BinarySplittingPiCalculator(
					new RamanujanBinarySplittingSeries(precision, radix)),
					precision, radix);
		}
		...
		protected RamanujanPiCalculator(BinarySplittingPiCalculator calculator,
				long precision, int radix) throws ApfloatRuntimeException {
			this.calculator = calculator;
			this.precision = precision;
			this.radix = radix;
		}
...
//			Pi.err.println("Using the Ramanujan binary splitting algorithm");
			...
			long terms = (long) ((double) this.precision
					* Math.log((double) this.radix) / 18.38047940053836);
...
//			Pi.err.printf("Final value ");
			...
			Apfloat t = T.getApfloat(), q = Q.getApfloat();
			checkAlive();
			Apfloat factor = ApfloatMath.inverseRoot(new Apfloat(8,
					this.precision, this.radix), 2);
			checkAlive();
			Apfloat pi = ApfloatMath.inverseRoot(t, 1).multiply(factor)
					.multiply(new Apfloat(9801, Apfloat.INFINITE, this.radix))
					.multiply(q);
			time = System.currentTimeMillis() - time;
...
//			Pi.err.println("took " + time / 1000.0 + " seconds");
	...
	public static class BinarySplittingProgressIndicator implements
			Serializable {
		/**
		...
		public BinarySplittingProgressIndicator(long terms) {
			this.totalElements = (long) (terms * (Math.log((double) terms)
					/ Math.log(2.0) + 1.0)) + 1;
		...
		public void progress(long n1, long n2) {
			long length = n2 - n1;
			// For small ranges the amount of progress is advanced in one chunk,
			// including the progress of all the recursive steps
			long addedElements;
		...
		private long recursiveLength(long length) {
			...
			if (length == 1) {
				...
				recursiveLength = recursiveLength(halfLength)
						+ recursiveLength(length - halfLength) + length;
			}
	...
	public static class GaussLegendrePiCalculator implements Operation<Apfloat> {
		...
		public Apfloat execute() {
			...
			long time = System.currentTimeMillis();
			Apfloat two = new Apfloat(2, this.precision, this.radix), four = new Apfloat(
					4, this.precision, this.radix), a = new Apfloat(1,
					this.precision, this.radix), b = ApfloatMath.inverseRoot(
					two, 2), t = a.divide(four);
			time = System.currentTimeMillis() - time;
			...
			for (int i = 0; i < iterations; i++) {
				...
				checkAlive();
				t = t.subtract(new Apfloat(1L << i, this.precision, this.radix)
						.multiply(ApfloatMath.pow(tmp.subtract(a), 2)));
		...
		private static long gaussLegendrePrecision(int k, int r, int radix) {
			return (long) ((Math.pow(2.0, (double) k) * Math.sqrt((double) r)
					* Math.PI - Math.log(16.0 * Math.sqrt((double) r)) - k
					* Math.log(2.0)) / Math.log((double) radix));
		}
	...
	public static class BorweinPiCalculator implements Operation<Apfloat> {
		...
		public Apfloat execute() {
			...
			long time = System.currentTimeMillis();
			Apfloat one = new Apfloat(1, this.precision, this.radix), two = new Apfloat(
					2, this.precision, this.radix), four = new Apfloat(4,
					this.precision, this.radix), y = ApfloatMath.sqrt(two)
					.subtract(one), a = two.subtract(four.multiply(y));
			time = System.currentTimeMillis() - time;
			...
			for (int i = 0; i < iterations; i++) {
				...
				checkAlive();
				a = a.subtract(new Apfloat(1L << (2 * i + 3), this.precision,
						this.radix).multiply(y).multiply(tmp.subtract(y)));
		...
		private static long borweinPrecision(int k, int r, int radix) {
			return (long) ((Math.pow(4.0, (double) k) * Math.sqrt((double) r)
					* Math.PI - Math.log(16.0 * Math.sqrt((double) r)) - k
					* Math.log(4.0)) / Math.log((double) radix));
		}
	...
	protected static long getLong(String arg, String name, long minValue,
			long maxValue) {
		long value = 0;
	...
	protected static int getInt(String arg, String name, int minValue,
			int maxValue) {
	...
	private static void dump() {
		...
		Pi.err.println("builderFactory = "
				+ ctx.getBuilderFactory().getClass().getName());
		...
		Pi.err.println("sharedMemoryTreshold = "
				+ ctx.getSharedMemoryTreshold());
...
//				+ " digits");
		...
		intent.putExtra("result", (result.length() > 1000) ? result.substring(0, 1000) : result);
		CPUSampleApp.getInstance().getApplicationContext().sendBroadcast(intent);
...
//		Pi.err.println("Total elapsed time " + time / 1000.0 + " seconds");
	...
	public static void main(String[] args) throws IOException,
		...
		int method = (args.length > 1 ? getInt(args[1], "method", 0, 3) : 0), radix = (args.length > 2 ? getRadix(args[2])
				: ApfloatContext.getContext().getDefaultRadix());
